        ...
        <div className="fixed inset-0 z-[60] grid place-items-center overflow-y-auto bg-neutral-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="tmdb-preview-title">
          <div className="app-panel my-8 w-full max-w-5xl p-6">
            ...
            {preview && <div className="mt-5 space-y-5">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-xl border border-neutral-200 p-4 dark:border-neutral-800"><h3 className="font-semibold">{t('targets.renamePlan')}</h3><p className="mt-2 break-all rounded-lg bg-neutral-50 p-3 font-mono text-xs dark:bg-neutral-900">{preview.plan.proposed_directory_path}</p>{preview.plan.proposed_file_renames.map((item) => <div key={item.source_path} className="mt-2 text-xs"><p className="break-all text-neutral-500">{item.source_path}</p><p className="break-all text-emerald-700 dark:text-emerald-300">→ {item.target_path}</p></div>)}</div>
                <div className="rounded-xl border border-neutral-200 p-4 dark:border-neutral-800"><h3 className="font-semibold">{t('targets.generatedFiles')}</h3><ul className="mt-2 space-y-2">{preview.plan.generated_files.map((file) => <li key={file} className="break-all font-mono text-xs text-neutral-600 dark:text-neutral-400">{file}</li>)}</ul></div>
              </div>

    ...
    <div className="mx-auto max-w-7xl space-y-5 px-4 py-8 sm:px-6 lg:px-8">
      ...
      {matchCandidate && <AppDialog open onOpenChange={(open) => { if (!open) setMatchCandidate(null) }} width="xl" closeLabel={t('common.close')} title={t('targets.tmdbPreview')} description={<span className="break-all font-mono text-xs">{matchCandidate.path}</span>}>
            ...
            {preview && <div className="mt-5 space-y-5">
              ...
              {(preview.plan.skipped_episodes?.length ?? 0) > 0 && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200"><h3 className="font-semibold">{t('targets.skippedEpisodes')}</h3><p className="mt-1 text-xs opacity-80">{t('targets.warnings.episodes_skipped')}</p><ul className="mt-2 space-y-2">{preview.plan.skipped_episodes.map((episode) => <li key={episode.source_path} className="break-all font-mono text-xs opacity-75">{episode.source_path}</li>)}</ul></div>}

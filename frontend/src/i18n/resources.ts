const en = {
  translation: {
    ...
    targets: {
      ...
      conflicts: { target_exists: 'A destination path already exists.', duplicate_target: 'Multiple source items map to the same destination.', video_missing: 'The candidate no longer contains a video file.', multiple_movie_videos: 'A movie candidate contains multiple video files and cannot be renamed safely.', season_ambiguous: 'A season directory contains conflicting season markers.', season_invalid: 'A season directory contains an invalid season marker.', duplicate_season_directory: 'Multiple directories map to the same season.', episode_unrecognized: 'A video file has no unambiguous season and episode marker.' },
    ...
    errors: {
      ...
      codes: {
        ...
        preview: { not_found: 'Scrape preview not found.', invalid_id: 'Preview ID is invalid.', invalid_snapshot: 'The stored preview snapshot is invalid.', stale: 'The directory changed after scanning. Run a new scan.' },
  ...
  translation: {
    ...
    errors: {
      ...
      codes: {
        ...
        preview: { not_found: '刮削预览不存在。', invalid_id: '预览 ID 无效。', invalid_snapshot: '保存的预览快照无效。', stale: '扫描后目录发生了变化，请重新扫描。' },

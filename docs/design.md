...
4. 扫描目标目录并形成媒体候选：
   - 电影：第一层电影目录；根目录平铺视频按 TMDB ID 或“标题+年份”保守归组，同一电影的多个清晰度/剪辑版本形成一个候选。
...
电影目录包含多个正片时，规划器为每个视频提取或接收可编辑的唯一版本标签，并生成 `标准电影名 - 版本标签.ext`。字幕和外部音轨跟随所属视频版本；版本专属旧 NFO 保留原名，统一生成目录级 `movie.nfo`。预告片、花絮和样片不计入版本，分段电影不与多版本模式混用，单部电影最多支持 8 个版本。

根目录平铺电影按 TMDB ID 优先、否则按“标准化标题+年份”形成候选；缺少年份且没有 TMDB ID 时仍按单文件处理。开启整理后，同组视频及其唯一关联伴随文件移动到同一个标准电影目录。无法自动得到唯一版本标签的候选保持阻断，用户可在预览中编辑标签后重新生成计划。
...
| `MEDIA_GID` | 空 | 可选的媒体目录附加组 ID，用于媒体共享组与主组不同的 NAS/宿主机 |
...
2. 电影根目录包含同一 TMDB ID 或相同标题年份的多个平铺版本时合并为一个候选；无可靠身份的信息仍保持独立。
